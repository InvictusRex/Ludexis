...
export default function AdminJobs() {
  const jobs = [
    { id: '1', name: 'Full Archive Scan', status: 'completed', progress: 100, startTime: '2 hours ago', icon: CheckCircle2, color: 'text-green-500' },
    { id: '2', name: 'Metadata Refresh', status: 'running', progress: 67, startTime: '30 minutes ago', icon: Clock, color: 'text-yellow-500' },
    { id: '3', name: 'Integrity Verification', status: 'queued', progress: 0, startTime: 'In queue', icon: AlertCircle, color: 'text-gray-500' },
  ]
        ...
        {['Queue', 'Running', 'Completed'].map((status) => (
          <Card key={status} className="border-border">
            <CardHeader>
              ...
              <CardDescription>{jobs.filter((j) => j.status.includes(status.toLowerCase())).length} jobs</CardDescription>
            </CardHeader>
          </Card>
        ))}
      </div>
